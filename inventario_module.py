...
def agregar_producto_catalogo(cursor, sku, nombre, precio, categoria_id):
    """Inserta un nuevo producto en el cat├ílogo."""
    try:
        ...
        print(" Producto agregado al cat├ílogo exitosamente.")
    except Exception as e:
        print(f" Error al insertar en la base de datos: {e}")

def ver_catalogo(cursor):
    """Devuelve la lista de productos del cat├ílogo."""
    try:
        cursor.execute("SELECT SKU, Nombre, PrecioVentaBase FROM Productos")
        productos = cursor.fetchall()
        return productos
    except Exception as e:
        print(f" Error al consultar el cat├ílogo: {e}")
        return []
...
def actualizar_precio_producto(cursor, sku, nuevo_precio):
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            print(f" No se encontr├│ ning├║n producto con el SKU: {sku}")
    except Exception as e:
